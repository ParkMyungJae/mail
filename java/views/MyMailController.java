...
public class MyMailController extends MasterController {
	@FXML
...
//	private ObservableList<String> item;
	...
	public void delete() {
		...
		String sql = "DELETE FROM `mail_item` WHERE WHERE owner = ? AND receive = ?";
	...
	public void reload() {
		...
		String sql = "SELECT `id`, `owner`, `title`, `date`  FROM `mail_item` WHERE owner = ? AND receive = ? ORDER BY `id` DESC";
