...
public class TodoController extends MasterController{
	...
	@FXML private ListView<TodoVO> todoList;
	@FXML private TextField todoTitle;
	@FXML private TextArea todoContent;
	...
	public void register() {
		...
		Connection con = JDBCUtil.getConnection();
		PreparedStatement pstmt = null;
		String sql = "INSERT INTO diary_todos(`title`, `content`, `date`, `owner`) "
					+ " VALUES(?, ?, ?, ?)";
		try {
			pstmt = con.prepareStatement(sql);
	...
	public void update() {
		...
		TodoVO todo = list.get(idx); //수정해야할 대상을 가져온다.
		
		Connection con = JDBCUtil.getConnection(); //데이터베이스 연결 가져옥
		PreparedStatement pstmt = null;
